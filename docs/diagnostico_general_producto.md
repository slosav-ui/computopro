...
**El diálogo de ajuste económico escribía `obras.monto_total` — RESUELTO 2026-09-10, era más grave de lo que parecía.** La columna que la 0091 dejó de usar como fuente de verdad, sí, pero la lectura original de este hallazgo se equivocaba en un punto: no era "nadie la relee" — el propio diálogo la releía como base para convertir de moneda, y como esa columna queda clavada en 0 para toda obra con cómputo real, **la conversión de USD a ARS daba 0** (bug real, encontrado por Seba al verificar la carga de la cotización BNA, no un riesgo latente). Corregido: la base de conversión pasa a los montos vivos (`montoEstimadoArs`/`montoEstimadoUsd`). Detalle completo en `docs/relevamiento_sincronizacion_config_precios.md` Hallazgo #3, actualizado.
