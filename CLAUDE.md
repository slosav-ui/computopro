...
nuevo); **Resumen es técnica y profunda** (números de la obra, avance por rubro, certificados,
adicionales con su detalle, gráficos). Dos reglas que resuelven los casos nuevos: en la portada se
recorta el **detalle**, nunca la **jerarquía de un monto cerrado**; y si un dato necesita una
aclaración para no malinterpretarse, ese dato no es de portada. Leer ese doc antes de agregarle o
sacarle algo a la card del dashboard, en vez de volver a discutirlo por pieza.
...
**Qué ves lo define el rol; qué editás, el permiso `puede_editar_presupuesto`** (migración 0121, `docs/etapa3_roles_permisos_diseno_datos.md` §10). Editar el presupuesto —cómputo, precios, Factor K e impuestos, vista del presupuesto, valor hora, orden de rubros, importar— y sus actos formales —presentar, congelar, emitir certificados, subir el PDF firmado, cerrar un certificado cobrado, anular, aprobar quitas/demasías, enviar un adicional— lo hace `admin_maestro` siempre, y `profesional`/`constructor` solo con ese permiso (default false, lo otorga solo admin_maestro al invitar o después). *"El que armó el presupuesto es el que lo edita; los demás lo ven pero no lo tocan."* (Seba). La regla vive en un helper SQL (`puede_editar_presupuesto(obra)`) que usan todas las políticas y funciones; `UserContext.puedeEditarPresupuesto` la espeja. Cargar avance, certificar avance de adicionales, solicitar adicionales/quitas/demasías y los libros siguen por rol.
- **Caja Blanca (100%)** — `admin_maestro`, `profesional`: ven todo (cómputos, APU, precios, coeficientes); editan según el permiso de arriba (el admin siempre).
- **Constructor** — `constructor`: ve lo mismo que el profesional (cómputo con precios, Mat y MO, APU oficial y su receta propia, Factor K de la obra, montos de Gestión de Obra y Adicionales, el audit_log completo) y carga avance; edita y emite con el permiso `puede_editar_presupuesto`, igual que el profesional. Sin el permiso es el caso del empleado que ejecuta: ve precios, compra y carga avance, no toca el presupuesto ni emite. Única diferencia que queda con el profesional en la base: no aprueba ajustes de contrato (`puede_aprobar_monto`, aprueba el que paga) — y los libros de Órdenes de Servicio/Notas de Pedido, que son de cada parte. **Cambio de matriz 2026-09-12**: antes era "Vista Operativa (sin montos) — constructor/capataz"; en el rubro argentino el constructor es la empresa que cotiza y ejecuta — *"es el que HACE EL PRESUPUESTO. Nunca es el capataz"* (Seba). El nombre significaba una cosa en el diseño y otra en el rubro. El capataz no es un rol: es alguien que el constructor invita. Detalle, diferencias con profesional y el límite del Factor K (es de la obra, no de la persona) en `docs/etapa3_roles_permisos_diseno_datos.md` §8.
...
- Admin Maestro → ve y edita todo → Caja Blanca (100%). Profesional → ve todo, edita con `puede_editar_presupuesto` (0121).
- Constructor → ve todo como el Profesional (cómputo **con precios**, APU oficial, Factor K de la obra), carga diaria de avance, edita y emite con `puede_editar_presupuesto` (cambio de matriz 2026-09-12 y 0121, ver arriba y `docs/etapa3_roles_permisos_diseno_datos.md` §8-§10; antes: "Constructor/Capataz → sin acceso a precios → Vista Operativa").
...
- **Ciclo de vida del Certificado de Obra (5 estados)** — definición completa: 1) *Borrador* (carga de avances por Profesional/Empresa), 2) *Emitido/Esperando Pago* (notifica al Propietario con fecha límite según plazo pactado), 3) *Leído por Propietario* (se notifica a la obra automáticamente al abrirse), 4) *Pagado por Propietario* (marca medio de pago + adjunta comprobante), 5) *Impactado y Cerrado* (la Empresa/Constructor verifica el cobro y adjunta factura final). Reglas especiales: el plazo de pago se configura **una sola vez antes del Certificado N°1** y aplica a todos los siguientes; si se opta por firma física, el sistema debe **bloquear la emisión del siguiente certificado** hasta subir el PDF/imagen firmado *(este bloqueo era el diseño original y se implementó tal cual — ver más abajo, sección "Ciclo de vida del Certificado de Obra (5 estados, Modelo A) — capa de datos completa", y `docs/certificados_ciclo_vida_diseno_datos.md` §11, para el cambio de criterio 2026-09 que lo sacó a propósito y lo reemplazó por un aviso no bloqueante en la UI)*.
...
**RLS applied in production** (`supabase/migrations/0004_rls_etapa3.sql`, confirmed active in the Supabase dashboard under Database → Policies): all 4 tables now enforce row-level access control at the database layer, built on 3 `SECURITY DEFINER` helper functions (`is_obra_member`, `tiene_rol_en_obra`, `puede_aprobar_monto`) rather than the single-owner pattern `obras` uses (`id_admin_creador = auth.uid()`), since these tables have multiple combinable roles per obra. This means unauthorized reads/writes are now blocked by Postgres itself, independent of the Flutter UI. What RLS does *not* do: column-level redaction — a role without amounts (today the Veedor; Constructor sees amounts since the 2026-09-12 matrix change) can still receive `monto_total`/`tope_monto_aprobacion` in any row it's allowed to read at all, since RLS filters rows, not columns. That's left to the app layer via `UserContext`'s getters.
...
- **`0011_certificados_funciones_transicion.sql`**: por la restricción de `0010`, las funciones de
  ...
  bloqueaba la emisión si el certificado anterior requería firma física y no se había subido el PDF
  firmado — **cambio de criterio 2026-09, Gestión de Obra pieza 4**: ese bloqueo se sacó a propósito
  en `0055_certificados_firma_fisica_no_bloquea.sql` porque la firma depende de terceros y puede
  tardar días; pasó a ser un aviso persistente y no bloqueante en la UI
  (`CartelFirmaPendiente`, Gestión de Obra), no algo que se perdió — ver
  `docs/certificados_ciclo_vida_diseno_datos.md` §11 para el detalle completo. `requiere_firma_fisica`
  y `subir_pdf_firmado_certificado` siguen funcionando exactamente igual, sin cambios),
